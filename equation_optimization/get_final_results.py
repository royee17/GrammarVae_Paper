...
if __name__ == '__main__':
    ...
    print(f"Mean: {np.mean(results_grammar, 0)}")
    print(f"Std: {np.std(results_grammar, 0) / np.sqrt(iteration)}")

    print(f"First: {np.min(results_grammar[ : , 1 ])}")
    best_score = np.min(results_grammar[ : , 1 ])
    results_grammar[ results_grammar[ : , 1 ] == best_score , 1 ] = 1e10
    print(f"Second: {np.min(results_grammar[ : , 1 ])}")
    second_best_score = np.min(results_grammar[ : , 1 ])
    results_grammar[ results_grammar[ : , 1 ] == second_best_score, 1 ] = 1e10
    print(f"Third: {np.min(results_grammar[ : , 1 ])}")
